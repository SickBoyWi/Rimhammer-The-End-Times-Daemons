    ...
    public class ThingSetMaker_MapGen_AncientDaemonPodContents : ThingSetMaker
    {
        protected override void Generate(ThingSetMakerParams parms, List<Thing> outThings)
        {
            ...
            switch (podContentsType2)
            {
                ...
                case PodContentsType.AncientFriendly:
                    outThings.Add((Thing)this.GenerateFriendlyAncient());
                    break;
                case PodContentsType.AncientIncapped:
                    outThings.Add((Thing)this.GenerateIncappedAncient());
                    break;
                case PodContentsType.AncientHalfEaten:
                    outThings.Add((Thing)this.GenerateHalfEatenAncient());
                    outThings.AddRange((IEnumerable<Thing>)this.GenerateScarabs());
                    break;
                case PodContentsType.AncientHostile:
                    outThings.Add((Thing)this.GenerateAngryAncient());
                    break;
                case PodContentsType.Slave:
                    outThings.Add((Thing)this.GenerateSlave());
                    break;
                default:
        ...
        private void GiveRandomLootInventoryForTombPawn(Pawn p)
        {
            if ((double)Rand.Value < 0.649999976158142)
                ...
                this.MakeIntoContainer((ThingOwner)p.inventory.innerContainer, ThingDefOf.Plasteel, Rand.Range(10, 50));
            if ((double)Rand.Value < 0.699999988079071)
                ...
                this.MakeIntoContainer((ThingOwner)p.inventory.innerContainer, ThingDefOf.ComponentSpacer, Rand.Range(-2, 4));
        }
